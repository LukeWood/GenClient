defmodule GenClient do
  defmacro __using__(for: module, calls: calls, casts: casts) do
    ...
    client_calls = Enum.map(calls,
      &GenClient.Client.call_definition/1
    )
    client_casts = Enum.map(casts,
      &GenClient.Client.cast_definition/1
    )

    server_calls = Enum.map(calls,
      &GenClient.Server.call_definition(module, &1)
    )
    server_casts = Enum.map(casts,
      &GenClient.Server.cast_definition(module, &1)
    )
